...
export default class MathsController extends Controller {
  ...
  get(id) {
    let response = {};
    let x = this.HttpContext.path.params.x !== undefined ? parseFloat(this.HttpContext.path.params.x) : undefined;
    let y = this.HttpContext.path.params.y !== undefined ? parseFloat(this.HttpContext.path.params.y) : undefined;
    let n = this.HttpContext.path.params.n !== undefined ? parseFloat(this.HttpContext.path.params.n) : undefined;
  ...
  post(data) {
    this.HttpContext.response.JSON({ error: "POST is not allowed for this endpoint" }, 200);
  }

  put(data) {
    this.HttpContext.response.JSON({ error: "PUT is not allowed for this endpoint" }, 200);
  }

  remove(id) {
    this.HttpContext.response.JSON({ error: "DELETE is not allowed for this endpoint" }, 200);
  }
}
